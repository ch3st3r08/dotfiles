...
def main():
    ...
    # File paths
    css_file = os.path.expandvars(
        "$HOME/.config/rofi/themes/gtk/gtk-base.rasi")
...
def replace_hypr_config(hypr_path, bg3_color, output_path=None):
    ...
    clean_color = bg3_color.replace(
        'ff', '') if bg3_color.endswith('ff') else bg3_color
    ...
    if re.search(pattern, content):
        ...
        try:
            with open(output_path, 'w') as file:
                file.write(content)
            print(f"Successfully replaced first rgba in hypr config: {
                  rgba_color}")
